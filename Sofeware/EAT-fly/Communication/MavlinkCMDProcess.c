...
static void Cmd10_MAV_CMD_SEND_WGA( uint8_t port_index , const mavlink_message_t* msg )
{
	...
	uint32_t wga;
	wga = MS_Attitude_get_WGA(ind);
	if( wga == 0 )
		wga = MS_Position_get_WGA(ind);
	
	bool wga_success = MS_Attitude_WGA() && MS_Position_WGA();
...
static void Cmd176_MAV_CMD_DO_SET_MODE( uint8_t port_index , const mavlink_message_t* msg )
{
	const mavlink_command_long_t* msg_rd = (mavlink_command_long_t*)msg->payload64;
	...
	if
	( 
		( current_Mode == 1 && Is_Calibration_Mode(msg_rd->param2) ) ||	//当前为地面模式，允许进入校准模式
		( Is_Calibration_Mode( current_Mode ) && msg_rd->param2 == 1 )	//当前为校准模式，允许进入地面模式
	)
	{	//
		change_Mode( msg_rd->param2 );
	}
}
...
void (*const Mavlink_CMD_Process[])( uint8_t port_index , const mavlink_message_t* msg_rd ) = 
{
	...
	/*174-*/	0	,
	/*175-*/	0	,
	/*176-*/	Cmd176_MAV_CMD_DO_SET_MODE	,
	/*177-*/	0	,
	/*178-*/	0	,
	/*179-*/	0	,
	/*180-*/	0	,
