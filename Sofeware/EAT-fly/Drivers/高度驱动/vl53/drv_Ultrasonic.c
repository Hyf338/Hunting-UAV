...
void init_drv_Ultrasonic()
{
	...
	UARTIntRegister(UART7_BASE,Ultrasonic_Handler);//UART中断地址注册	
				...
				if(vl53_sum == rdata)
				{
					if(((vl53_byte6 >> 4) & 0x0f) == 0)//距离可靠
					{
						...
						if(vl53_position.z > 3 && vl53_position.z < 300)//中距离
						{
							...
							PositionSensorUpdatePosition( default_ultrasonic_sensor_index , vl53_position , true , -1 );
						}
					}
					else
