	...
	while( ( UART1->FR & (1<<4) ) == false	)
	{
		...
		{	//接收包尾
			if( rdata == 0x55 )
			{
				...
				float ultra_height;
				//定义两个BOOL型
				bool ultra_height_available_0 = get_Estimated_Sensor_Position_z( &ultra_height , default_GY53_sensor_index );
				bool ultra_height_available_1 = get_Estimated_Sensor_Position_z( &ultra_height , default_ultrasonic_sensor_index );
				
				
				//判断哪个传感器可用
				if( ultra_height_available_0 && Flow.valid )
				{
          ...
          if( ultra_deadband < 5 )
				...
				else if ( ultra_height_available_1 && Flow.valid )
				{
					float ultra_deadband = ultra_height - 5;
          if( ultra_deadband < 5 )
						ultra_deadband = 0;
