 ...
 */
MAVLINK_HELPER uint16_t mavlink_msg_to_send_buffer(void (*write_port)( const uint8_t* data , uint16_t length ), const mavlink_message_t *msg)
{
	...
	if (msg->magic == MAVLINK_STX_MAVLINK1) {
		...
		buf[5] = msg->msgid & 0xFF;
		write_port( &buf[0] , 6 );
		write_port( (uint8_t*)_MAV_PAYLOAD(msg) , msg->len );
	} else {
		...
		buf[9] = (msg->msgid >> 16) & 0xFF;
		write_port( &buf[0] , 10 );
		write_port( (uint8_t*)_MAV_PAYLOAD(msg), length);
		signature_len = (msg->incompat_flags & MAVLINK_IFLAG_SIGNED)?MAVLINK_SIGNATURE_BLOCK_LEN:0;
	}
	...
	write_port( &buf[0] , 2 + signature_len );
